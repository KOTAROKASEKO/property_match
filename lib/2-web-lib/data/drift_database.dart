...
class ChatDao extends DatabaseAccessor<AppDatabase>
  ...
  ChatThread _mapRowToChatThread(ChatThreadRow row) {
    pr('--- Mapping ChatThreadRow ---');
    pr('id: ${row.id} (Type: ${row.id.runtimeType})');
    pr('whoSent: ${row.whoSent} (Type: ${row.whoSent.runtimeType})');
    pr('whoReceived: ${row.whoReceived} (Type: ${row.whoReceived.runtimeType})');
    pr('timeStamp: ${row.timeStamp} (Type: ${row.timeStamp.runtimeType})');
    // 必要に応じて他のフィールドも追加
    print('-----------------------------');
    return ChatThread()
      ..id = row.id
...
@override
  ...
  Stream<List<ChatThread>> watchChatThreads() {
    pr('watchChatThreads called in DriftChatRepository');
